...
program define xtdcce2fast, eclass sortpreserve
	syntax varlist(min=1 ts) [if/] , [cr_lags(numlist) CRosssectional(varlist ts) noconstant lr(string) fullsample NOTable lr_options(string) NOCROSSsectional cd]
	qui{
		...
		** run regressions
		sort `idvar' `tvar'
		tempname b_output V_output stats b_i stats_i
		
		mata m_xtdcce2fast("`varlistnew'","`clistfull'","`BaseIndex'","`touse'","`idvar' `tvar'","`b_output' `V_output' `stats'",`b_i'=.,`stats_i'=.)
...
mata:
							...
							
	{
		...
		if (LongRunBases!= "") {
			...
			LongRunBases = LongRunBases[selectindex(LongRunBases:!=0)]
			...
			while (i<=rows(uniqBase)) {
				...
				if (s==1) {
					omega_lag = 1:-quadrowsum(b_i[.,index])
					b_i = b_i , -omega_lag
				}
				else {
