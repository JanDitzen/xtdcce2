...
program define xtcse2, rclass
		...
		*** Result matrix
		...
		foreach res in `varl' {
			...
			***eit and mub
			...
			mata `eitm' = `xx' - `PC'*invsym(quadcross(`PC',`PC')) * quadcross(`PC',`xx')
	...
	*** Output
	...
	foreach var in `varn' {
		xtdcce_output_table `var' `col_i' `alpha_circ' `alpha_circSE' `tm' cv `var'	
	}
...
mata:
								...
								real scalar a_size)
	{
		...
		coef = invsym(tmp_xx1)*quadcross(xbart_I,xm)
		etmp = xm - xbart_I * coef
		s2 = etmp'etmp:/(T-cols(xbart_I))
		se = sqrt(diagonal(s2*invsym(quadcross(xbart,xbart))))
		t_test = (coef[2,.]:/se[1,.])'
		size = J(N,1,0)
		x_str = J(T,1,0)
		order = (1::N)
		...
		dv=v_1[.,1]
		rhs=v_1[.,(2..p+1)]
		b=invsym(quadcross(rhs,rhs))*quadcross(rhs,dv)
		s_b=colsum(b)
		e_nw=dv-rhs*b
		sse_nw=e_nw'*e_nw
		...
		c_avg = invsym(quadcross(x_bar,x_bar))*quadcross(x_bar,xm)
